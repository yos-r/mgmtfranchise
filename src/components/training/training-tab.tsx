...
export function TrainingTab() {
  ...
  // Function to load a single event by ID
  const loadSingleEvent = async (id: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Failed to load the training event details",
        variant: "destructive",
      });
      navigate('/training');
    } finally {
  ...
  // Handle selecting an event and updating the URL
  const handleSelectEvent = (event: any) => {
    setSelectedEvent(event);
    navigate(`/training/${event.id}`, { replace: true });
  };

  // Handle going back to the events list
  ...
  // Training Content component to wrap everything
